...
```
### 返回值
addFunction('func') ，目标函数 func 可以直接返回结果值，也可以返回`Promise`对象，`Hprose`将取`Promise`的`resolve`值返回给客户端，或`reject`返回错误信息。也可以用`yield`语句返回`Generator`对象，`Hprose`将取最后一个`yield`的值返回给客户端。
